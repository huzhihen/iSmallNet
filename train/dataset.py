...
class Data(Dataset):
    ...
    def __getitem__(self, idx):
        if self.cfg.datapath == '../data/NUAA-SIRST':
            name  = self.samples[idx]
            name1 = name + "_pixels0"
            tag = True
        elif self.cfg.datapath == '../data/NUDT-SIRST':
            name  = self.samples[idx]
            name1 = name
            tag = False
        ...
        if self.cfg.mode=='train':
            ...
            body   = cv2.imread(self.cfg.datapath+'/body-origin/' + name1 + '.png', 0).astype(np.float32)
            detail = cv2.imread(self.cfg.datapath+'/detail-origin/' + name1 + '.png', 0).astype(np.float32)
